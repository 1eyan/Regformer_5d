...
def apply_rotary_pos_emb_nd(
    ...
    if omega_bands is not None:
        omega_bands = omega_bands.to(dtype=q.dtype, device=q.device)
        expected = (coord_dim, part_dim // 2)
        if tuple(omega_bands.shape) != expected:
            raise ValueError(
                f"omega_bands shape must be {expected}, got {tuple(omega_bands.shape)}"
            )
        pos = coords
    else:
        max_coord = 1000.0
...
class RotaryPositionEmbeddingND(nn.Module):
    ...
    def __init__(
        ...
        self.base = base
        if omega_bands is None:
            self.omega_bands = None
        else:
            omega_bands = torch.as_tensor(omega_bands, dtype=torch.float32)
            expected = (coord_dim, (head_dim // coord_dim) // 2)
            if tuple(omega_bands.shape) != expected:
                raise ValueError(
                    f"omega_bands shape must be {expected}, got {tuple(omega_bands.shape)}"
                )
            self.register_buffer("omega_bands", omega_bands, persistent=True)
    ...
    def forward(
        ...
        """
        return apply_rotary_pos_emb_nd(
            q, k, coords, self.coord_dim, self.base, omega_bands=self.omega_bands
        )
...
class GatedMultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model, n_heads, dropout=0.1,
        ...
        if use_rope:
            ...
            self.rope_nd = RotaryPositionEmbeddingND(
                self.head_dim, coord_dim=6, omega_bands=rope_omega_bands
            )
        else:
            self.rope = None
            self.rope_nd = None
...
class GatedTransformerEncoderBlock(nn.Module):
    ...
    def __init__(self, d_model, n_heads, d_ff, dropout=0.1,
        ...
        self.attn = GatedMultiHeadAttention(
            ...
            use_rope=use_rope,
            rope_omega_bands=rope_omega_bands
        )
